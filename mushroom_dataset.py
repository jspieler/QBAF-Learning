...
with open('mushroom_results.csv', 'w') as file:
...
for params in parameters.values():
    ...
    for run in range(params['number_runs']):
        ...
        best_fitness = []
        mean_fitness = []
        patience = 5  # number of generations to wait before stopping genetic algorithm
        count = 0
        threshold = 1e-4
        best_score = None
        ...
        with open('mushroom_results.csv', 'a') as file:
            writer = csv.writer(file)
            writer.writerow(
