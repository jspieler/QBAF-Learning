...
with open('iris_results_layerwise_crossover.csv', 'w') as file:
...
for params in parameters.values():
    ...
    for run in range(params['number_runs']):
        ...
        for g in range(str_ln.generations):
            ...
            pc1 = torch.empty(size=(len(crossover_offspring), 12 * params['hidden_size']))
            pc2 = torch.empty(size=(len(crossover_offspring), params['hidden_size'] * 3))
        ...
        with open('iris_results_layerwise_crossover.csv', 'a') as file:
            writer = csv.writer(file)
            writer.writerow(
